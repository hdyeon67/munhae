// 시드 해시 + 시드 기반 결정적 유틸 (bujeok/saju-core hash.ts 를 그대로 재사용)
//
// 입력 문자열을 FNV-1a 32bit 해시로 시드화하고, mulberry32 로 결정적 난수를 뽑는다.
// 같은 입력이면 항상 같은 시드/난수가 나오므로 재계산해도 결과가 동일하다.
// munhae 는 사주 도메인이 없어 saju-core 에 의존하지 않고 원시함수를 여기에 직접 둔다.
 ...
 * 태그가 시드를 네임스페이스화하므로 서로 다른 선택이 상관되지 않는다. (bujeok deriveIndex 동일)
 */
