...
describe("선택지 셔플 — 결정성 · 정답 재매핑 · 위치 분포", () => {
  it("같은 시드에서 선택지 순서가 결정적으로 동일", () => {
    const seed = 424242;
    const a = generatePaper(FIXTURE_POOL, seed, "daily");
    const b = generatePaper(FIXTURE_POOL, seed, "daily");
  ...
  it("재매핑된 answerIndex 는 항상 원래 정답 텍스트를 가리킨다", () => {
    for (const seed of sampleSeeds()) {
      for (const it of generatePaper(FIXTURE_POOL, seed, "daily").items) {
  ...
  it("셔플이 선택지를 실제로 섞는다 (원본 순서 보존이 아님)", () => {
    // 편향 픽스처(대부분 정답 index 1)라도, 표본 전체에서 최소 한 번은 순서가 바뀐다
    let reordered = 0;
    for (const seed of sampleSeeds()) {
      for (const it of generatePaper(FIXTURE_POOL, seed, "daily").items) {
  ...
  it("정답 위치가 ①~④에 고르게 분포한다 (쏠림 무력화)", () => {
    ...
    // 이상적 균등 = 25%. 편향 픽스처였음에도 각 위치가 12~40% 범위에 들어오면 쏠림이 깨진 것.
